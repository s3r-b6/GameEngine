...
std::vector<FrontTile> *loadCollisions(BumpAllocator *bumpAlloc) {
    std::vector<FrontTile> *collisions =
        new (bumpAlloc->alloc(sizeof(std::vector<FrontTile>))) std::vector<FrontTile>();
    ...
    for (int i = 0; i < TILES_CHUNK_x * TILES_CHUNK_y; i++) {
        ...
        engine_log("Collision %u %d %d", id, x, y);
        FrontTile t = {};
        t.posX = x, t.posY = y;
        t.id = id;
